"""Apply neon/migrations/*.sql in order against DATABASE_URL (alias NEON_DATABASE_URL).

For the Supabase-only pilot, point DATABASE_URL at the Supabase direct Postgres
connection string; this applies the full application schema there.
"""
...
# Neon-only files — skip when applying to Supabase Postgres.
...
def main() -> None:
    ...
    if not url:
        raise SystemExit(
            "DATABASE_URL (or NEON_DATABASE_URL, or SUPABASE_URL + SUPABASE_DB_PASSWORD) "
            "is not set in .env"
        )
