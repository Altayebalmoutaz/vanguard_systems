"""Orchestrate scribe suggest: idempotency → propose → leftover LLM → validate → persist."""
...
from app.coding.catalog import cdt_metadata, validate_codes
...
from app.coding.guards import apply_clinical_guards, void_recommendation
from app.coding.pending import remember_pending_run
...
from app.tools.coding_tools import apply_payer_rules_tool
...
def run_coding_suggest(
    ...
    *,
    ...
    schedule_persist: Callable[[Callable[[], None]], Any] | None = None,
) -> CodingSuggestResponse:
    ...
    idempotency_ms = int((time.perf_counter() - _t_idem) * 1000)
    ...
    proposed = propose(request)
    propose_ms = int((time.perf_counter() - _t_propose) * 1000)
    resolved_by_id = {p.line_id: p for p in proposed if p.resolved}
    unresolved_ids = [p.line_id for p in proposed if not p.resolved]

    _t_codegen = time.perf_counter()
    llm_by_id: dict[str, dict[str, Any]] = {}
    if unresolved_ids:
        leftover_req = request.model_copy(
            update={
                "procedures": [p for p in request.procedures if p.line_id in set(unresolved_ids)]
            }
        )
        leftover_note = build_clinical_note(leftover_req)
        leftover_ids = [p.line_id for p in leftover_req.procedures]
        try:
            memory = ""
            if use_retrieval and supabase is not None and app_settings.jina_api_key:
                ...
                retrieval_ms = int((time.perf_counter() - _t_ret) * 1000)
            _t_llm = time.perf_counter()
            llm_called = True
            generated = llm_generate_line_recommendations(
                app_settings,
                structured_block=structured_prompt_block(leftover_req),
                clinical_note=leftover_note,
                patient_age=age,
                insurance=insurance,
                line_ids=leftover_ids,
            ...
            llm_recs = list(generated.get("recommendations") or [])
            ...
            llm_recs, overall_confidence, justification = _flat_fallback(
                app_settings,
                cfg,
                note=leftover_note,
                age=age,
                insurance=insurance,
                request=leftover_req,
                supabase=supabase,
                use_retrieval=use_retrieval,
                warnings=warnings,
            )
        llm_by_id = {str(r.get("line_id")): r for r in llm_recs}
    else:
        justification = "Deterministic chairside rules"
    code_gen_ms = int((time.perf_counter() - _t_codegen) * 1000)

    line_recs_raw = []
    for proc in request.procedures:
        hit = resolved_by_id.get(proc.line_id)
        if hit is not None:
            line_recs_raw.append(hit.as_rec())
            continue
        line_recs_raw.append(
            llm_by_id.get(proc.line_id)
            or {
    ...
    catalog = validate_codes(
        cdt_codes,
        icd_codes,
        supabase=supabase,
        ttl_seconds=cfg.coding_reference_cache_ttl_seconds,
    )
    validate_ms = int((time.perf_counter() - _t_val) * 1000)
    if catalog.loaded:
        for code in catalog.invalid_cdt:
            warnings.append(f"CDT {code} not in reference catalog")
        for code in catalog.invalid_icd:
            warnings.append(f"ICD-10 {code} not in dental reference catalog")
    ...
    invalid_cdt = set(catalog.invalid_cdt)
    invalid_icd = set(catalog.invalid_icd)
    _t_ref = time.perf_counter()
    cdt_meta = cdt_metadata(
        cdt_codes,
        supabase=supabase,
        ttl_seconds=cfg.coding_reference_cache_ttl_seconds,
    )
    reference_ms = int((time.perf_counter() - _t_ref) * 1000)
    ...
    cmap: CalibrationMap = []
    autonomy_allowlist: frozenset[str] = frozenset()
    ...
    for proc in request.procedures:
        ...
        if cdt_norm and cdt_norm in invalid_cdt:
            void_recommendation(
                raw,
                f"CDT {cdt_norm} is not in the reference catalog; cannot recommend it.",
            )
            warnings.append(f"Voided invalid CDT {cdt_norm} on line {proc.line_id}")
            cdt_norm = None
        try:
        ...
        recommendations.append(
            LineRecommendation(
                ...
                icd10_codes=[
                    icd
                    for c in (raw.get("icd10_codes") or [])
                    if (icd := str(c).upper().strip()) and icd not in invalid_icd
                ],
    ...
    def _persist(persist_ms: int = 0) -> None:
        insert_coding_run(
            app_settings,
            practice_id=practice_id,
            request_id=request.request_id,
    ...
    if schedule_persist is not None:
        schedule_persist(_persist)
    else:
        _t_persist = time.perf_counter()
        _persist()
        # Sync path still measured; audit already wrote persist_ms=0 which is
        # acceptable — chairside latency is coding time, not the write.
        _ = int((time.perf_counter() - _t_persist) * 1000)
